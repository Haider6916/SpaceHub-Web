...
import { closeEditEventDrawer, eventsReloadTrue } from '../../../Redux/eventsSlice';
import { LocalizationProvider, StaticTimePicker } from '@mui/x-date-pickers';
import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs';
import moment from 'moment';
import MapModal from '../MapModal/MapModal';
import '../AddEventsDrawer/AddNewEvents.css'
...
function EditEventsDrawer() {

    const classes = useStyles();
    const dispatch = useDispatch()

    const eventDetailsForEdit = useSelector((state) => state.event.eventDetailsForEdit)
    ...
    //Image Upload
    const [image, setImage] = useState(eventDetailsForEdit.eventPicture)
    ...
    const formik = useFormik({
        initialValues: {
            title: eventDetailsForEdit.title,
            startDate: eventDetailsForEdit.startDate,
            endDate: eventDetailsForEdit.endDate,
            from: eventDetailsForEdit.startTime,
            to: eventDetailsForEdit.endTime,
            description: eventDetailsForEdit.description,
            where: eventDetailsForEdit.location,
            link: eventDetailsForEdit.locationLink,
            seats: eventDetailsForEdit.seatsLimit,
        },
        ...
        onSubmit: () => {
            ...
            privateRequest.put(`/event/${eventDetailsForEdit._id}`, formData, {
                headers: {
    ...
    const convertTime = (timeString) => {
        const momentObj = moment(timeString, 'H:mm:ss.SSSZ');
    ...
    useEffect(() => {
        formik.setFieldValue('startDate', convertDateFormat(eventDetailsForEdit.startDate))
        formik.setFieldValue('endDate', convertDateFormat(eventDetailsForEdit.endDate))

        formik.setFieldValue('from', convertTime(eventDetailsForEdit.startTime))
        formik.setFieldValue('to', convertTime(eventDetailsForEdit.endTime))
    }, [])
    ...
    return (
        <form onSubmit={formik.handleSubmit}>
            <Box display='flex' justifyContent='space-between' alignItems='center'>
                ...
                <CloseCircle onClick={() => dispatch(closeEditEventDrawer())} style={{ cursor: 'pointer' }} />
            ...
            <Box sx={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end' }} >
                <Button
                    onClick={() => { dispatch(closeEditEventDrawer()) }}
                    variant='outlined'
                    sx={{ marginRight: '22.64px', fontStyle: 'normal', fontWeight: '500', fontSize: '16px', lineHeight: '28px', borderRadius: '51px', color: '#8D55A2', width: '166px', height: '40px' }}>
                    Back
                </Button>
                <Button
                    type='submit'
