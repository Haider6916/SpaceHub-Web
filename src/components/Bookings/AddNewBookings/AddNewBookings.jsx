...
const AddNewBookings = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: () => {
            ...
            let id = sessionStorage.getItem("bookingRoomID");

            privateRequest.post(`/booking/${id}`, apiObject).then((res) => {
    ...
    const formik1 = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            if (parseInt((selectedEmployees.length + visitors.length)) < parseInt(sessionStorage.getItem("bookingRoomCapacity"))) {
                const tempObj = {
    ...
    const imageLink = sessionStorage.getItem("bookingRoomPhoto")
    ...
    const fetchBookings = async () => {
        let id = sessionStorage.getItem("bookingRoomID");
        await privateRequest.get(`/booking/${id}`).then((res) => {
    ...
    const makeBookedSlots = (selectedDate) => {
        ...
        console.log('Booked Slots', tempArray);
    ...
    const bookMeetingRoom = () => {
        ...
        const apiObject = {
            ...
            "startTime": convertToISO(formik.values.date, formik.values.startTime),
            "endTime": convertToISO(formik.values.date, formik.values.endTime),
            "resource": sessionStorage.getItem("bookingRoomID"),
    ...
    return (
        ...
        <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <CloseCircle onClick={() => dispatch(closeAddBookingDrawer())} style={{ cursor: 'pointer' }} />
            </div>
            ...
            </Box>
            {
                (activeStep === 1) &&
                < >
                    <div style={{ width: "525.442px", height: "133.286px", borderRadius: '8px', border: "1px solid #9F9D9E" }}>
                        <img src={imageLink} alt="Loading Image" style={{ height: "inherit", width: "inherit", borderRadius: 'inherit' }} />
                    </div>

                    <Typography sx={{ color: '#8D55A2', fontSize: '24px', fontWeight: "500", lineHeight: '24px', mt: '16px', mb: '24px' }}>
                        {sessionStorage.getItem("bookingRoomName")}
                    </Typography>
                    <form onSubmit={formik.handleSubmit}>
                        <div style={{ gap: '24px', display: 'flex', flexDirection: 'column', width: '546px' }}>
                            ...
                            <div>
                                <Box onClick={(event) => { setOpenStartDate(!openStartDate) }} sx={{ position: 'relative', alignItems: "center", width: 'inherit', backgroundColor: formik.values.date === '' ? '#FAFAFA' : '#EEE5F1', borderRadius: '8px', height: '56px', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', cursor: 'pointer', '&:hover': { backgroundColor: '#EEE5F1' } }}>
                                    <Typography sx={{
...

            {
                ...
                <>
                    <Typography sx={{ fontWeight: "400", fontSize: '18px', color: '#272627' }}>
                        Attendees: {selectedEmployees.length + visitors.length}/{sessionStorage.getItem("bookingRoomCapacity")}
                    </Typography>
