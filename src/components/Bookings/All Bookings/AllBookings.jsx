...
import { Box, Button, Chip, IconButton, Menu, MenuItem, Modal, Typography } from '@mui/material'
...
const AllBookings = (props) => {
    ...
    const fetchAllBookings = () => {
        ...
        privateRequest.get('/booking/all-bookings', {
            headers: {
                'x-pagination-skip': 1,
        ...
        privateRequest.get('/booking/occupied-meeting-rooms').then((res) => {
            props.setTopInfo(res.data.count)
        }).catch((error) => {
            toast.error(error.response.data.error.message);
        })
    }

    const fetchMoreAllBookings = () => {
        let searchUrl = sessionStorage.getItem("searchURL");


        privateRequest.get(searchUrl === '' ? '/booking/all-bookings' : searchUrl, {
            headers: {
    ...
    const fetchAllBookingsByFilter = () => {
        ...
        privateRequest.post('/booking/all-bookings', apiObject, {
            headers: {
                'x-pagination-skip': 1,
                'x-pagination-limit': 10
            }
    ...
    const fetchMoreAllBookingsByFilter = () => {
    ...
    return (
        <>
            ...
            <Box sx={{ marginBottom: '16.8px' }}  >
                <SearchBar placeholderValue='Search All Bookings' handleSetValue={(value) => handleSearch(value)} />
            </Box>
            ...
            <InfiniteScroll
                ...
                next={isFiltered ? fetchMoreAllBookingsByFilter : fetchMoreAllBookings}
