...
function AddEmployeeDrawer(props) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            privateRequest.post('/user/employee', apiObject).then((res) => {
                toast.success(res.data.message)
                dispatch(employeeReloadTrue())
                props.handleCloseAddEmployee()
            }).catch((error) => {
                toast.error(error.response.data.error.message);
            }).finally(() => {
                setLoader(false)
        ...
        <>
            <form onSubmit={formik.handleSubmit}>
                <Box display='flex' justifyContent='space-between' alignItems='center'>
                    ...
                    <CloseCircle onClick={() => props.handleCloseAddEmployee(false)} style={{ cursor: 'pointer' }} />
                ...
                <Box sx={{ marginTop: '100px', display: 'flex', justifyContent: 'flex-end' }} >
                    <Button
                        onClick={() => props.handleCloseAddEmployee(false)}
